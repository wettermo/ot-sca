...
def decrypt_stepwise(ciphertext, key, exit_tuple=(0, 'addkey')):
    ...
    if exit_tuple[1] not in [
            'addkey', 'inv_shiftrows', 'inv_mixcolums', 'inv_subbytes'
    ]:
        raise ValueError(
            "AES-Operation must be 'addkey', 'inv_shiftrows', 'inv_mixcolums', 'inv_subbytes'"
        )
...
def encrypt_stepwise(plaintext,
                     key,
                     exit_tuple=(10, 'addkey'),
    ...
    if exit_tuple[1] not in ['addkey', 'shiftrows', 'mixcolums', 'subbytes']:
        raise ValueError(
            "AES-Operation must be 'addkey', 'shiftrows', 'mixcolums', 'subbytes'"
        )
    ...
    for i in range(1, 10, 1):
        ...
        if (i == 9 and fault == True):
            state[faulted_byte_index] = fault_model(state[faulted_byte_index],
                                                    fault_type)
...
def encrypt_fault(plaintext,
    ...
    if fault_type not in [
        ...
        exit(1)
    return encrypt_stepwise(plaintext, key, (10, 'addkey'), faulted_byte_index,
                            True, fault_type)
...
def verilator_sim_data(verilator_out_file):
    eff_faults = 0
    ineff_faults = 0
    ineff_ciphertexts = []
    ineff_plaintexts = []
    ret_key = (int("0x00", 16)).to_bytes(16, byteorder='little')

    with open(verilator_out_file, 'r', newline='') as csvfile:
        reader = list(csv.reader(csvfile))
        row_count = sum(1 for r in reader) - 1

        progress_bar = tqdm.tqdm(total=row_count, ncols=80)
        progress_bar.set_description(
            'Evaluating faulty AES encryptions from Verilator simulation')

        for row in reader:
            if row[0] == "Plaintext":  # omit title row
                pass
            else:
                plaintext = "0x" + row[0]
                ciphertext = "0x" + row[1]
                key = "0x" + row[2]
                expected_ciphertext = "0x" + row[3]

                # Note that the byte order in OpenTitan AES is little endian
                key = (int(key, 16)).to_bytes(16, byteorder='little')
                ret_key = key

                plaintext = list((int(plaintext,
                                      16)).to_bytes(16, byteorder='little'))
                ciphertext = list((int(ciphertext,
                                       16)).to_bytes(16, byteorder='little'))
                expected_ciphertext = list(
                    (int(expected_ciphertext,
                         16)).to_bytes(16, byteorder='little'))
                calculated_plaintext = decrypt(ciphertext, list(key))

                if plaintext == calculated_plaintext:
                    ineff_faults += 1
        ...
        if (total_faults == 0):
            print("Ineffective rate:  ", 0)
        else:
            print("Ineffective rate:  ", ineff_faults / total_faults)

        return ineff_ciphertexts, ineff_plaintexts, ret_key
...
def sim_data(faulted_byte_index, fault_type, numb_runs):
    ...
    for i in range(numb_runs):
        _, text = ktp.next()
        ciphertext = encrypt_fault(list(text), list(key), faulted_byte_index,
                                   9, fault_type)
...
def fault_ciphertexts_from_plaintext(faulted_byte_index, fault_type, numb_runs,
                                     plaintexts, key):
    eff_faults = 0
    ineff_faults = 0
    ineff_ciphertexts = []
