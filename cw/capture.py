...
def capture_init(ctx, num_traces, plot_traces):
    ...
    if "use_fixed_key_iter" in cfg["capture"]:  # for backwarts compatibility
        ...
        # if fixed_key = true, then key never updates and setting...
...
def check_ciphertext(ot, expected_last_ciphertext, ciphertext_len):
    ...
    actual_last_ciphertext = ot.target.simpleserial_read("r", ciphertext_len, ack=False, timeout=5000)
    assert actual_last_ciphertext == expected_last_ciphertext[0:ciphertext_len], (
        f"Incorrect encryption result!\n"
        f"actual  : {binascii.b2a_hex(actual_last_ciphertext)}\n"
        f"expected: {binascii.b2a_hex(expected_last_ciphertext)}"
    )
...
def capture_otbn_vertical(ot, ktp, fw_bin, pll_frequency, capture_cfg):
    ...
    # register ctrl-c handler to not loose already recorded traces if measurement is aborted
